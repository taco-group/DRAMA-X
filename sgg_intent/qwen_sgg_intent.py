...
class Qwen2_5OpenRouterInference:
    def __init__(
        self,
        dataset_path: str,
        api_key: str = None,
        openai_key: str    = None,
        model_name: str = "qwen/qwen-2.5-vl-7b-instruct",
        max_tokens: int = 512,
        ...
        self.scene_graph_prompt = self._scene_graph_prompt()
        self.intent_prompt      = self._intent_prompt()
    ...
    def _intent_prompt(self):
        ...
        an example structure would look like this: dictionaru of {"name_of_object": {"Intent": ["predicted lateral intent", "predicted vertical intent"], "Reason": "reason for this prediction", "Bounding_box": [x1, y1, x2, y2] }} for all objects and NOT a list
    ...
    def _all_gen_prompt(self):
        ...
        an example structure would look like this for given scene, dictionary of {"Risk": Yes/No, "Suggested_action": "suggested action for ego vehicle", "pedestrian": {"Intent": ["predicted lateral intent", "predicted vertical intent"], "Reason": "reason for this prediction", "Bounding_box": [x1, y1, x2, y2] }, "car": {"Intent": ["predicted lateral intent", "predicted vertical intent"], "Reason": "reason for this prediction", "Bounding_box": [x1, y1, x2, y2] },...(for all objects and NOT a list)} 
        The Intent field list should ALWAYS have two values: one for lateral and one for vertical. Strictly output in valid JSON format. This JSON shoud NOT contain details from the scene graph such as the relationships or attributes, stick to the format mentioned above.
        """
        
        return all_gen_prompt
    def encode_image_to_base64(self, image_url):
        resp = requests.get(image_url, stream=True, timeout=self.http_timeout)
        resp.raise_for_status()
        data = resp.content
        img  = Image.open(BytesIO(data))
        return base64.b64encode(data).decode('utf-8'), img.height, img.width
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                return r.json()
                    ...
                    ProtocolError) as e:
                # Transient network / chunk issues → retry
    ...
    def process_image_for_scene_graph(self, image_url):
        b64, h, w = self.encode_image_to_base64(image_url)
        messages = [{
            "role":"user",
            "content":[
                {"type":"image_url", "image_url":{"url":f"data:image/jpeg;base64,{b64}"}},
                {"type":"text", "text":f"{self.scene_graph_prompt} Original dims: h={h}, w={w}. Respond ONLY JSON."}
            ]
        }]
        resp = self.call_openrouter_api(messages)
        ...
        # print(out)
        try:
            out = self.extract_and_fix_json(out, prompt_type="scene")
            return out
        except json.JSONDecodeError:
            print("Scene graph JSON parse error")
            return {}
    ...
    def process_image_for_intent(self, image_url, scene_graph):
        ...
        messages = [{
            ...
            "content":[
                {"type":"image_url", "image_url":{"url":f"data:image/jpeg;base64,{b64}"}},
                {"type":"text", "text":f"{self.intent_prompt} Original dims: h={h}, w={w}. Scene graph: {json.dumps(scene_graph)}"}
            ]
        }]
        resp = self.call_openrouter_api(messages)
        out  = resp["choices"][0]["message"]["content"]
        if out.startswith("```json"): out = '{' + out[9:]
        if out.endswith("```"):       out = out[:-5] + '}'
        # print(out)
        
        try:
            out = self.extract_and_fix_json(out, prompt_type="intent")
            return out
        except json.JSONDecodeError:
            print("Intent JSON parse error")
            return {}
    def process_image_one_pass(self, image_url):
        ...
        messages = [{
            ...
            "content":[
                {"type":"image_url", "image_url":{"url":f"data:image/jpeg;base64,{b64}"}},
                {"type":"text", "text":f"{self.all_gen_prompt_template} Original dims: h={h}, w={w}.  Analyse this image"}
            ]
        }]
        resp = self.call_openrouter_api(messages)
        out  = resp["choices"][0]["message"]["content"]
    ...
    def run_inference(self, limit=None, overwrite=True, raw = False):
        ...
        data = self.load_data()
        out_dir = os.path.join(os.path.dirname(self.dataset_path), "outputs", "qwen_openrouter")
        ...
        if limit:
            items = items[:limit]
        if raw:
            raw_path = os.path.join(out_dir, "all_raw_op1.json")
...
if __name__ == "__main__":
    # either set OPENROUTER_API_KEY in env or pass it here
    os.environ["OPENROUTER_API_KEY"] = "<open-router-api-key>"  

    inference = Qwen2_5OpenRouterInference(
        dataset_path="drama_intent/updated_output.json",
    ...
    # test with limit=10 then remove limit"
    sg, intents = inference.run_inference(overwrite = False, limit = None, raw = True)
